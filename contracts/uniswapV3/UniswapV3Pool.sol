...
contract UniswapV3Pool is IUniswapV3Pool, NoDelegateCall {
    ...
    // THIS FUNCTION WAS MADE JUST TO UPDATE THE PRICE ON TESTNET DEMO
    function updatePrice(uint160 sqrtPriceX96) external onlyFactoryOwner() {
        int24 tick = TickMath.getTickAtSqrtRatio(sqrtPriceX96);
    ...
    function swap(
        ...
        while (state.amountSpecifiedRemaining != 0 && state.sqrtPriceX96 != sqrtPriceLimitX96) {
            ...
            if (exactInput) {
                ...
                unchecked { state.amountSpecifiedRemaining -= (step.amountIn + step.feeAmount).toInt256(); }
                state.amountCalculated -= step.amountOut.toInt256();
            } else {
                unchecked { state.amountSpecifiedRemaining += step.amountOut.toInt256(); }
            ...
            if (state.sqrtPriceX96 == step.sqrtPriceNextX96) {
                ...
                if (step.initialized) {
                    ...
                    unchecked { if (zeroForOne) liquidityNet = -liquidityNet; }
        ...
        if (zeroForOne) {
            ...
            unchecked { if (state.protocolFee > 0) protocolFees.token0 += state.protocolFee; }
        } else {
            feeGrowthGlobal1X128 = state.feeGrowthGlobalX128;
            unchecked { if (state.protocolFee > 0) protocolFees.token1 += state.protocolFee; }
        }
        ...
        if (zeroForOne) {
            unchecked { if (amount1 < 0) TransferHelper.safeTransfer(token1, recipient, uint256(-amount1)); }

            uint256 balance0Before = balance0();
            IUniswapV3SwapCallback(msg.sender).uniswapV3SwapCallback(amount0, amount1, data);
            // if (balance0Before + uint256(amount0) > balance0()) revert IIA();
        } else {
            unchecked { if (amount0 < 0) TransferHelper.safeTransfer(token0, recipient, uint256(-amount0)); }
